...
export default function ClickSpeedPage() {
  const [bests, setBests] = useState<Record<number, number | null>>(() => {
    const initialBests: Record<number, number | null> = {};
    TIME_MODES.forEach(mode => {
      initialBests[mode] = getBest(mode);
    });
    return initialBests;
  });
  ...
  const liveCps = phase === 'playing'
    ? parseFloat((clicks / elapsed).toFixed(1))
    : parseFloat((clicks / timeMode).toFixed(1));
  ...
  // Result screen
  if (phase === 'result') {
    const finalCps = parseFloat((clicks / timeMode).toFixed(1));
    return (
      <>
        <Navbar />
        <TestResult
          ...
          scoreSubtitle={`${clicks} total clicks in ${timeMode} seconds`}
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 flex flex-col pt-16">
        ...
        {/* ── INTRO ── */}
        {phase === 'intro' && (
          <div className="flex-1 flex flex-col items-center justify-center px-4 py-8 md:py-10">
            <div className="text-center max-w-md w-full">
              ...
              <p className="text-blue-300 text-sm md:text-lg mb-7 md:mb-8 max-w-sm mx-auto">
                ...
                <div className="flex justify-center gap-2 flex-wrap">
                  {TIME_MODES.map(t => (
                    <button
                      ...
                      id={`time-mode-${t}`}
                      className={`px-4 md:px-5 py-2.5 rounded-full font-black text-sm transition-all border ${
                        timeMode === t
              ...
              <button
                id="start-click-test"
                onClick={startGame}
                className="w-full py-4 md:py-5 rounded-2xl font-black text-lg md:text-xl text-white bg-gradient-to-r from-blue-500 to-cyan-500 shadow-2xl shadow-blue-500/30 hover:opacity-90 hover:scale-105 transition-all btn-breathe"
              >
                Start Clicking! 🖱️
              </button>
        ...
        {/* ── PLAYING ── */}
        {phase === 'playing' && (
          <div className="flex-1 flex flex-col items-center justify-center px-4 py-6 md:py-8 select-none">

            {/* Stats row */}
            <div className="grid grid-cols-3 gap-2 md:gap-3 mb-5 w-full max-w-sm">
              {/* Timer */}
              <div className={`bg-white/10 border rounded-2xl p-2.5 md:p-3 text-center ${timerUrgent ? 'border-red-500/50' : 'border-white/15'}`}>
                <div className={`text-2xl md:text-3xl font-black ${timerUrgent ? 'text-red-400 timer-pulse' : 'text-white'}`}>
                  {timeLeft}
                </div>
              ...
              <div className="bg-white/10 border border-white/15 rounded-2xl p-2.5 md:p-3 text-center">
                <div className="text-2xl md:text-3xl font-black" style={{ color: tier.color }}>{clicks}</div>
                <div className="text-white/40 text-xs font-bold">clicks</div>
              </div>

              {/* Live CPS */}
              <div className="bg-white/10 border border-white/15 rounded-2xl p-2.5 md:p-3 text-center">
                <div className="text-2xl md:text-3xl font-black text-cyan-400">{liveCps}</div>
            ...
            {/* Tier badge */}
            <div
              className="mb-4 px-4 md:px-5 py-2 rounded-full font-black text-xs md:text-sm transition-all"
            ...
            >
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <div
                  className="text-6xl sm:text-7xl md:text-9xl font-black leading-none transition-all duration-75"
                  style={{ color: tier.color, textShadow: `0 0 40px ${tier.color}` }}
                >
                  {clicks}
                </div>
                <div className="text-white/40 text-sm md:text-base font-bold mt-1">TAP ANYWHERE</div>
